...
from avici.haiku import lift # custom copy-paste from dm-haiku==0.0.6.dev0
...
class _LayerStack(module.Module):
  ...
  def __call__(self, x, *args_ys):
    ...
    remat_apply_fn = jax.remat(apply_fn)
    def layer(carry: LayerStackCarry,
      ...
      if self._remat:
        out_x, z = remat_apply_fn(params, rng, carry.x, *scanned.args_ys)
      else:
...
class _LayerStackNoPerLayer(_LayerStack):
  ...
  def __init__(self,
               ...
               remat: bool,
               name: Optional[str] = None):
    super().__init__(count=count, unroll=unroll, remat=remat, name=name)
    _check_no_varargs(f)
    self._f = f
    ...
    if with_per_layer_inputs:
      ...
      def wrapped(x, *args):
        ...
        return _LayerStackWithPerLayer(
            f, num_layers, unroll=unroll, remat=remat, name=name)(x, *args)
    else:
      _check_no_varargs(f)
      @functools.wraps(f)
      def wrapped(*args):
        ret = _LayerStackNoPerLayer(
            f, num_layers, unroll=unroll, remat=remat, name=name)(args, None)[0]
        if len(args) == 1:
