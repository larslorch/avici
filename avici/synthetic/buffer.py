...
class Sampler:
    ...
    def generate_data(rng, *, n_vars, spec_list, spec=None):
        ...
        g, effect_sgn, toporder = spec.g(rng, n_vars)

        # sample mechanism mechanism and data
        data = spec.mechanism(spec=spec, rng=rng, g=g, effect_sgn=effect_sgn, toporder=toporder, n_vars=n_vars)

        # check dims and standardize values
        if data["x_obs"].ndim == 2:
            data["x_obs"] = onp.expand_dims(data["x_obs"], axis=-1)
        if data["x_int"].ndim == 2:
            data["x_int"] = onp.expand_dims(data["x_int"], axis=-1)
